...
/**
 * 代表关系操作的行输出,允许按照索引的通用获取方法,这个会导致包装原语的开销和本地原语的获取.
 * 使用本地接口去检索一个空值是非法的,相反应付使用@isNullAt 在检索值之前
 * 为了创建一个新的行,使用@RowFactory.create() 或者@Row.apply()
 * 行对象的构建示例
 * {{{
 * import org.apache.spark.sql._
 *
...
trait Row extends Serializable {
  ...
  /**
   * 获取指定位置i处的值,如果value为空则返回null.显示是spark sql 类型与java类型的转换
   *
   * {{{
   *   BooleanType -> java.lang.Boolean
   *   ByteType -> java.lang.Byte
